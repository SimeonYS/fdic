...
class FdicPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fdic`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fdic WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fdic (date, title, link, content)"
